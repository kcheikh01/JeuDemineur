...
public class Minesweeper extends JFrame implements ActionListener {
	...
	// Messages qui apparaîtront dans des boîtes de dialogue
	
	Object lostMsg = "Vous avez touché une mine. Game Over. Bonne chance pour la prochaine fois!";
	Object wonMsg = "Bravo, Vous avez réussi!\nTime: "; 
	Object restartMsg = "Es tu sur de vouloir recommencer?";
	
	// Noms des images utilisées
	
	String[] imagenames = {"", "flag.png", "question_mark.png", "cross.png", "mine.png", "mine1.png", "clock.png"};
	
	// Tableau d'objets Icone pour les images en haut de la fenêtre
	public Icon[] upperImages;
	
	 // Image pour la mine qui sera utilisée
	...
	// Panel principal contenant tous les boutons
	// Panel en bas de la fenêtre pour afficher le temps et le nombre de mines
	...
	// Menu déroulant pour les options de difficulté
	
	JMenu optionsmenu, thememenu;
	JMenuItem aboutitem;
	JMenuBar menubar;
	String[] optionsString = {"Debutant(10 mines, 9 x 9)", "Intermediate(40 mines, 16 x 16)", "Expert(99 mines, 16 x 30)"};
	...
	// Tableau de points contenant la position de toutes les mine
	
	Point[] mine;
	// Fonction principale qui crée la fenêtre du jeu
	...
	public Minesweeper() {
		...
		super("Démineur"); // Nom de la fenêtre
		upperImages = new Icon[7]; // Chargement des images
		for (int i = 0; i < upperImages.length; i++) {
			
			upperImages[i] = new ImageIcon(getClass().getResource("/images/" + imagenames[i]));
		}
		mineImage = new ImageIcon(getClass().getResource("/images/" + imagenames[5])).getImage(); //chargement des images
		...
		aboutitem = new JMenuItem("A propos", KeyEvent.VK_U);
		aboutitem.addActionListener(this);
		optionsmenu.add(aboutitem);
		menubar.add(optionsmenu);
		setJMenuBar(menubar);
		startGame(); //demarre le jeu
		setVisible(true);
	...
	private void startGame() {
		...
		// Définir le layout et les bordures du panneau du jeu.
		...
		// Créer les boutons nécessaires pour le jeu.
		
		createButtons();
		
		// Créer le panneau des scores.
	...
	private void gameWon() {
		...
		clock.stop();
		openAllButtons();
		Object[] options = {"Rejouer", "Exit"};
		int selection = JOptionPane.showOptionDialog(this, wonMsg + timerText.getText() + " seconds", "Vous avez gagné", JOptionPane.YES_NO_OPTION, JOptionPane.PLAIN_MESSAGE, null, options, options[0]);
	...
	public void gameLost() {
		...
		openAllButtons();
		Object[] options = {"Rejouer", "Exit"};
		int selection = JOptionPane.showOptionDialog(this, lostMsg, "Vous avez perdu", JOptionPane.YES_NO_OPTION, JOptionPane.PLAIN_MESSAGE, null, options, options[0]);
		if (selection == JOptionPane.YES_OPTION) {
			size = getSize();
//			remove(gamePanel);
			remove(scorePanel);
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		if (arg0.getSource() == aboutitem) {
			...
			JOptionPane.showMessageDialog(this, "Développé par KD & Nasser.", getTitle(), JOptionPane.INFORMATION_MESSAGE);
		} else {
